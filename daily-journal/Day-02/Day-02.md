...
![alt text](Session-multiplexing.png)
...
Sender sends data traffic with source port number and destination port number, when receiver is sending back traffic, the destination port number and source port number are flipped around. A type of firewall uses this to block unauthorised traffic, so it only always allows traffic from the sender to the receiver but when the receiver sends back it compares, or rather checks, if the connection was initiated by a sender or receiver, if it was initiated by a receiver then the traffic is blocked, so it checks if the sender destination port number = the receiver source port number and vice versa, if they don't match it means connection was initiated by receiver so traffic is blocked
![alt text](port-numbers.png)
...
![alt text](connection-setup.png)

UDP header:
![alt text](UDP-header.png)
